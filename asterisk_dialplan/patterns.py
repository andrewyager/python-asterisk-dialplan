...
def generate_patterns(low, high):
    ...
    Will generate a DialplanException if first_number and last_number are not of equal
    length
...
def _chunk_safe_patterns(pattern):
    """
    Take a pattern string and split out out into pattern strings where only the
    least significant
    digits change.
    """
...
def _get_lowest_number_from_pattern(pattern):
    ...
    # We should be matching for patterns that look like [234589] here as well and
    # finding the minimum
